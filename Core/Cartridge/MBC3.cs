...
public class MBC3 : ICartridge
{
    ...
    public MBC3(byte[] rom, string cartridgeName)
    {
        ...
        if (_battery && File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + $"/GEGC/saves/{_romName}.save"))
        {
            using FileStream stream = File.OpenRead(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + $"/GEGC/saves/{_romName}.save");
    ...
    public void SaveRam()
    {
        if (_battery)
        {
            Directory.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "/GEGC/saves/");
            using FileStream stream = File.OpenWrite(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + $"/GEGC/saves/{_romName}.save");
