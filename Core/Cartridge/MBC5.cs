...
public class MBC5 : ICartridge
{
    ...
    public MBC5(byte[] rom, string cartridgeName)
    {
        ...
        {
            _sram = File.ReadAllBytes(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + $"/GEGC/saves/{_romName}.save");
        }
    }
    ...
    public void SaveRam()
    {
        if (_battery && _sram != null)
        {
            ...
            File.WriteAllBytes(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + $"/GEGC/saves/{_romName}.save", _sram);
        }
    }
